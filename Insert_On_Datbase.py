...
def insert_in_Local(get_htmlSource , SegFeild):
    ...
    while MyLoop == 0:
        mydb = DB_connection()
        mycursor = mydb.cursor()
        sql = "INSERT INTO asia_tenders_tbl(Tender_ID,EMail,add1,Country,Maj_Org,tender_notice_no,notice_type,Tenders_details,short_desc,est_cost,currency,doc_cost,doc_last,earnest_money,Financier,tender_doc_file,source,URL,cpv)VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
...
def insert_L2L(SegFeild , Fileid):
    ...
    Col1 = 'https://www.tenderwizard.my/'
    if SegFeild[7] == "IN":
